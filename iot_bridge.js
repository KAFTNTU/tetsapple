...
(function() {
  ...
  // - HUB: watchdog stop if drive packets stop arriving (safety, без "інерції")
  // - Remote->HUB: тунель BLE write через MQTT (для Scratch/bytecode/команд)
  let lastDriveRxTs = 0;
  let stopWatchdogTimer = null;
  const DRIVE_STOP_TIMEOUT_MS = 220;

  function ensureStopWatchdog() {
    if (stopWatchdogTimer) return;
    stopWatchdogTimer = setInterval(function() {
      if (mode !== 'hub') return;
      if (!window.isConnected || typeof window.sendDrivePacket !== 'function') return;
      if (!lastDriveRxTs) return;
      if (Date.now() - lastDriveRxTs > DRIVE_STOP_TIMEOUT_MS) {
        lastDriveRxTs = 0;
        try { window.sendDrivePacket(0, 0, 0, 0); } catch(e) {}
      }
    }, 50);
  }
  ...
  function handleMqttMessage(topic, message) {
    ...
    if (mode === 'hub' && topic.indexOf(TOPIC_CONTROL) === 0) {
      ...
      try {
        ...
        } else if (data && data.type === 'ble' && data.b64) {
          if (!window.isConnected) return;
          const bytes = b64ToBytes(data.b64);
          if (typeof window._bleWrite === 'function') {
            try { window._bleWrite(bytes); } catch (e) {}
          } else if (window.characteristic && typeof window.characteristic.writeValue === 'function') {
            try { window.characteristic.writeValue(bytes).catch(()=>{}); } catch (e) {}
          }
        }
      } catch(e) {}
    }
  ...
  function publishDrive(l, r) {
    ...
    const now = Date.now();
    if (now - lastDrivePublish < 20) return;
  ...
  function publishBleWrite(bytes) {
    if (!mqttClient || !mqttConnected || mode !== 'remote' || !selectedRemoteId) return;
    const b64 = bytesToB64(bytes);
    if (!b64) return;
    mqttClient.publish(TOPIC_CONTROL + selectedRemoteId, JSON.stringify({type:'ble',b64,ts:Date.now()}), {qos:0,retain:false});
  }
  ...
  window.processScratchCommandLocal = function(cmd) {
    // HUB side: execute Scratch text-command (if transport exposes it)
    const text = (cmd == null) ? '' : String(cmd);
    if (typeof window.sendTextData === 'function') {
      try { window.sendTextData(text); } catch(e) {}
    }
  };

  // Keep original modal layout on iOS: user enters IoT mode via the small apple button.
  ...
  IOT.getMode           = function() { return mode; };
  IOT.isRemoteReady     = function() { return mode === 'remote' && !!selectedRemoteId && !!mqttConnected; };
  IOT.publishBleWrite   = publishBleWrite;
