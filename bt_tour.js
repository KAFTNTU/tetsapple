(() => {
  ...
  const STEPS_IOS = [
    {
      title: "iPhone: IoT режим",
      text: "На iPhone Web Bluetooth майже завжди не працює. Тому підключення робимо через IoT (MQTT) + Hub.",
      note: "Далі покажу, де саме увімкнути IoT та як ввести Room Code.",
    },
    {
      title: "Відкрий меню підключення",
      text: "Натисни кнопку BT у верхньому правому куті, щоб відкрити вікно підключення.",
      selector: "#btConnect",
      autoNextOnClick: "#btConnect",
      onShow() {
        closeBtModalSafe();
    ...
    {
      title: "Натисни IoT (з яблучком)",
      text: "У вікні підключення натисни кнопку IoT. Вона відкриє IoT Remote режим.",
      selector: "#iotHubBtn",
      autoNextOnClick: "#iotHubBtn",
      ensureBtModalOpen: true,
      onShow() {
        openBtModalSafe();
      },
    ...
    {
      title: "Введи Room Code",
      text: "Введи Room Code та натисни «Підключитись». Код має бути однаковий на Hub і на iPhone.",
      selector: "#iotRemoteCodeInput",
      ensureBtModalOpen: true,
      onShow() {
        openBtModalSafe();
      },
    ...
    {
      title: "Обери машинку",
      text: "Після підключення з'явиться список машинок. Обери свою машинку і керуй як звичайно.",
      selector: "#iotDeviceList",
      ensureBtModalOpen: true,
      onShow() {
        openBtModalSafe();
      },
    ...
    {
      title: "Як пройти навчання ще раз",
      text: "Якщо потім забудеш, як підключатися, відкрий Налаштування і натисни кнопку «Пройти навчання ще раз».",
      note: "Кнопка навчання завжди буде в кінці списку налаштувань.",
      selector: "#settingsTutorialBtn",
      onShow() {
        closeBtModalSafe();
        if (typeof window.openSettings === "function") window.openSettings();
  ...
  const STEPS = IS_IOS ? STEPS_IOS : STEPS_BT;
...
`
  ...
  function renderStep() {
    ...
    byId("raStepHead").textContent = `${IS_IOS ? "IOT" : "BLUETOOTH"} НАВЧАННЯ ${state.stepIndex + 1}/${STEPS.length}`;
